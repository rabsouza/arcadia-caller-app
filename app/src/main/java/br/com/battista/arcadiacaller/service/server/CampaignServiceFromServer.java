...
public class CampaignServiceFromServer extends BaseService implements CampaignService {
    ...
    public static final String TAG = CampaignServiceFromServer.class.getSimpleName();

    @Override
    public List<Campaign> findByUser(@NonNull String token, @NonNull String username) {
        Log.i(TAG, MessageFormat.format("Find campaigns by username: {0} in app server url:[{1}]!",
        ...
        try {
            ...
            if (response != null && response.code() == HttpStatus.NO_CONTENT.value()) {
                Log.i(GifHeaderParser.TAG, "Found 0 campaigns!");
            } else if (response != null && response.code() == HttpStatus.OK.value() && response.body() != null) {
                campaigns = Lists.newArrayList(response.body());
                Log.i(GifHeaderParser.TAG, MessageFormat.format("Found {0} campaigns!", campaigns.size()));
                return campaigns;
            } else {
    ...
    public Campaign create(@NonNull String token, @NonNull Campaign campaign) {
        ...
        Log.i(TAG, MessageFormat.format("Create new campaign with alias: {0} in app server url:[{1}]!",
        ...
        try {
            ...
            if (response != null && response.code() == HttpStatus.CREATED.value() && response.body() != null) {
                ...
                Log.i(GifHeaderParser.TAG, MessageFormat.format("Create campaign: {0}!", campaign));
                return campaign;
            } else {
                String errorMessage = MessageFormat.format(
    ...
    public Campaign update(@NonNull String token, @NonNull Campaign campaign) {
        ...
        Log.i(TAG, MessageFormat.format("Update new campaign with key: {0} in app server url:[{1}]!",
        ...
        try {
            ...
            if (response != null && response.code() == HttpStatus.OK.value() && response.body() != null) {
                campaign = response.body();
                Log.i(GifHeaderParser.TAG, MessageFormat.format("Update campaign: {0}!", campaign));
                return campaign;
            } else {
                String errorMessage = MessageFormat.format(
    ...
    public Boolean delete(@NonNull String token, @NonNull Campaign campaign) {
        ...
        Log.i(TAG, MessageFormat.format("Delete campaign with key: {0} in app server url:[{1}]!",
        ...
        try {
            ...
            if (response != null && response.code() == HttpStatus.OK.value()) {
                Log.i(GifHeaderParser.TAG, MessageFormat.format("Success deleted campaign: {0}!", campaign));
                return Boolean.TRUE;
            } else {
                String errorMessage = MessageFormat.format(
